...
public class StringValidator {
    ...
    public boolean isValidType(String input, Pattern pattern, String chartype) {
        if (pattern.matcher(input).matches()) {
        	errorList.add(String.format("文字の種類が不適切です。%sのみ使用できます。", chartype));
            return false;
        }
        return true;
    }
    ...
    public boolean isValidLength(String input, int minLength, int maxLength) {
        if (input.length() < minLength || input.length() > maxLength) {
        	errorList.add("文字の長さが不適切です。" + minLength + "〜" + maxLength + "文字で入力してください。");
            return false;
        }
        return true;
    }
    ...
    // すべてのチェックを実行
    public boolean validate(String input, int minLength, int maxLength, int checkType) {
    	String checkRegex;
    	String charType;
    	switch(checkType) {
    	// case 0:
        ...
        return isNotEmpty(input) && isValidType(input, pattern, charType) && isValidLength(input, minLength, maxLength);
    }
